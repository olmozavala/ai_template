...
def single_conv_lay_3d(input, num_filters, filter_size, batch_norm=False, dropout=False):
...
def multiple_conv_lay_3d(input, num_filters, filter_size, make_pool=True, batch_norm=False, dropout=False, tot_layers=2):
    ...
    for c_layer_idx in range(tot_layers):
        c_input = single_conv_lay_3d(c_input, num_filters, filter_size, batch_norm=batch_norm, dropout=dropout)

    if make_pool:
        maxpool = MaxPooling3D(pool_size=(2, 2, 2))(c_input)
    else:
        maxpool = []
...
def make_multistream_3d_unet(inputs, num_filters=8, filter_size=3, num_levels=3,
    ...
    for c_stream in range(tot_streams):
        ...
        for level in range(num_levels):
            ...
            conv_t, pool_t = multiple_conv_lay_3d(c_input, filters, filter_size, make_pool=True,
    ...
    [conv_p, dele] = multiple_conv_lay_3d(merged, filters, filter_size, make_pool=False, batch_norm=batch_norm_decoding,
    ...
    for level in range(1,num_levels+1):
        ...
        [conv_p, dele] = multiple_conv_lay_3d(merged, filters, filter_size, make_pool=False,
...
def make_inverted_multistream_3d_unet(inputs, tot_streams, num_filters, filter_size, num_levels):
    '''
    This function makes inverted 3D-Unets. One single inputstream and multiple output streams
    :param inputs:
    :param tot_streams:
    :param num_filters:
    :param filter_size:
    ...
    streams = []
    print(F"Building analysis path.... ")
    m_pool = True
    c_input = inputs
    convs = []
    maxpools = []
    # Making single stream down
    for level in range(num_levels):
        filters = num_filters * (2 ** level)
        conv_t, pool_t = multiple_conv_lay_3d(c_input, filters, filter_size, make_pool=m_pool, batch_norm=batch_norm,
                                              dropout=dropout)
        print(F"Filters {filters} Conv (before pool): {conv_t.shape} Pool: {pool_t.shape} ")
        convs.append(conv_t)
        maxpools.append(pool_t)
    ...
    # Conv Layers at the bottom
    filters = num_filters * (2 ** (level + 1))
    [conv_p, dele] = multiple_conv_lay_3d(c_input, filters, filter_size, make_pool=False, batch_norm=batch_norm,
                                          dropout=dropout)
    print(F"Filters {filters} Conv (before deconv): {conv_p.shape}")

    print("\n ------------- DECODING ----------------")

    # Multiple streams going up
    outputs = []
    for c_stream in range(tot_streams):
        print(F"----------- Stream {c_stream} ----------- ")
        last_input_stream = Conv3DTranspose(filters, (2, 2, 2), strides=(2, 2, 2), activation='relu',
                                            padding='same')(conv_p)
        for level in range(1, num_levels+1):
            bottom_up_level = num_levels-level
            filters = num_filters * (2 ** (bottom_up_level))
            # Merge with the proper level
            merged = concatenate([convs[bottom_up_level],last_input_stream])
            print(F'Merged size: {merged.shape}')
            [last_input_stream, _] = multiple_conv_lay_3d(merged, filters, filter_size, make_pool=False,
                                                          batch_norm=batch_norm, dropout=dropout)
            print(F"Filters {filters} Conv (before deconv): {conv_p.shape}")
            if level != (num_levels):
                last_input_stream = Conv3DTranspose(filters, (2, 2, 2), strides=(2, 2, 2), activation='relu',
                                                    padding='same')(last_input_stream)
                print(F"Filters {filters} Conv (after deconv): {conv_t.shape}")

        output_stream = Conv3D(1, (1, 1, 1), activation='sigmoid')(last_input_stream)
        print(F"Final shape for this stream {output_stream.shape}")
        outputs.append(output_stream)

    return outputs

def make_multistream_3d_half_unet_for_classification(inputs, num_filters=8, filter_size=3, num_levels=3,
                                                     size_last_layer=10,
    ...
    filters = num_filters * (2 ** (num_levels))
    [conv_p, dele] = multiple_conv_lay_3d(merged, filters, filter_size, make_pool=False, batch_norm=batch_norm,
                                          dropout=dropout)
    # Dense layers
